...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const { name, professor_name, description, location, available_sessions } = body;
        const { id } = await params;
        db.prepare(
            'UPDATE labs SET name=?, professor_name=?, capacity=?, description=?, location=?, available_sessions=? WHERE id=?'
        ).run(name, professor_name, 5, description, location, available_sessions || '1,2,3', id);
